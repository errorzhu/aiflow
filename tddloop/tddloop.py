...
def cmd_init(args, cfg):
    """初始化项目：生成 tddloop.json 和 skill 文件"""
    import shutil
    ...
    if config_path.exists():
        ...
        if not overwrite:
            print("⚡ tddloop.json 已存在。使用 --force 覆盖。")
            return

    # 生成 tddloop.json
    json_content = {
        "_comment": "tddloop 项目配置 — 定义技术栈相关的命令模板和路径",
        "python_prefix": "",
        "app_entry": "src/main.py",
        "test_runner": "pytest {test_file} -v",
        "test_runner_all": "pytest tests/ -v",
    ...
    print(f"✓ 生成 tddloop.json")

    # 复制 tddloop.py 到项目根目录
    myself = Path(__file__).resolve()
    dest_py = PROJECT_ROOT / "tddloop.py"
    if myself != dest_py:
        shutil.copy2(str(myself), str(dest_py))
        os.chmod(str(dest_py), 0o755)
        print(f"✓ 复制 tddloop.py")

    # 初始化 skill 文件
    # 优先从 tddloop.py 所在目录（发布包）读取，否则从 CODEX_HOME 读取
    pkg_dir = Path(__file__).resolve().parent
    skill_src = None
    for candidate in [
        pkg_dir,                                          # 发布包目录
        Path(os.environ.get("CODEX_HOME", os.path.expanduser("~/.codex"))) / "skills" / "tddloop",
    ]:
        sk = candidate / "SKILL.md"
        if sk.exists():
            skill_src = candidate
            break

    if skill_src:
        # 复制到项目 .codex 目录（非沙箱限制）
        ...
        if skill_md.exists():
            shutil.copy2(str(skill_md), str(codex_dir / "SKILL.md"))
            print(f"✓ 复制 skill SKILL.md → .codex/")

        # 复制 agents 目录
        skill_agents = skill_src / "agents"
        if skill_agents.exists():
            dest_agents = codex_dir / "agents"
            if not dest_agents.exists():
                shutil.copytree(str(skill_agents), str(dest_agents))
                print(f"✓ 复制 skill agents/  → .codex/agents/")

        print()
        print("🎉 tddloop 初始化完成！")
        print()
        print("  Skill 文件已放入 .codex/ 目录。")
        print("  如需让 Codex 识别此 skill，复制到 .agents/ 目录：")
        print("    cp -r .codex/SKILL.md .codex/agents/ .agents/")
        print()
        print("  下一步：")
        print("    python3 tddloop.py status                查看验证条件")
        print('    python3 tddloop.py run "你的需求描述"    开始 TDD 循环')
    else:
        print()
        print(f"⚠️  未找到 tddloop skill（{skill_src}），仅初始化了工具文件。")
        print()
        print("🎉 tddloop 工具初始化完成！")
        print()
        print("  下一步：")
        print("    python3 tddloop.py status                查看验证条件")
        print('    python3 tddloop.py run "你的需求描述"    开始 TDD 循环')
...
def cmd_export(args, cfg):
    ...
    for c in conditions:
        ...
        for case in c["cases"]:
            ...
            lines += [
                f"- {mark} `{case['id']}`  {case.get('description', case.get('desc', ''))}",
...
def run_shell(cmd, cfg, cwd=None):
    ...
    cmd_parts = shlex.split(prefix) if prefix else []
...
def _generate_test_file(condition, cfg):
    ...
    for case in condition["cases"]:
        ...
        desc_short = case.get("description", case.get("desc", "")).split("—")[0].strip().replace(" ", "_").lower()
        ...
        lines += [
            ...
            f'    """{case["id"]}: {case.get("description", case.get("desc", ""))}"""',
            f"    # TODO: implement test for: {case.get('input') or case.get('verification') or '—'}",
            f"    # Expected: {case.get('expected_output') or case.get('verification') or '—'}",
            f"    pass",
            "",
        ]
...
def cmd_run(args, cfg):
    ...
    for case in uncovered_cases:
        print(f"    ○ {case['id']}  {case.get('description', case.get('desc', ''))}")
...
def _auto_mark_done(condition_id, cfg):
    ...
    save_conditions(data, cfg)
    cmd_export(argparse.Namespace(), cfg)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  tddloop init                          初始化项目（生成配置 + skill）
  tddloop init --force                  强制覆盖已有配置
...
""",
    ...
    # init
    p_init = sub.add_parser("init", help="初始化项目（生成 tddloop.json + skill 文件）")
